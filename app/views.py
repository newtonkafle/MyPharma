...
from .models import Contact, Customer, Product, Cart, OrderPlaced,Contact
...
def search(request):
	...
	if query != '':
		products = Product.objects.filter(Q(title__contains=query)|Q(salt_composition__contains=query)|Q(brand__contains=query))
		num_articles = len(products)
		print(num_articles)
		for i in products:
			print(i.product_image)
	return render(request, 'app/search.html', {'query': query, 'products': products, 'num_results': num_articles})
